...
#if defined(SUPLA_SERVO_CUSTOM)
  for (int i = 0; i < ConfigManager->get(KEY_MAX_SERVO)->getValueInt(); i++) {
    new SuplaServoDimmer(i);
  }
#endif
...
void addRGBWLeds(uint8_t nr) {
  ...
  if (buttonPin != OFF_GPIO) {
    ...
    if (rgbwButtonManager.buttonRGBW.find(nrButton) == rgbwButtonManager.buttonRGBW.end()) {
      auto button = Supla::Control::GUI::Button(buttonPin, ConfigESP->getPullUp(buttonPin), ConfigESP->getInversed(buttonPin), nr);
...
void addCSE7766(int8_t pinRX) {
  if (counterCSE7766 == NULL && pinRX != OFF_GPIO) {
    HardwareSerial &uart = ConfigESP->getHardwareSerial(pinRX, -1, CSE7766_BAUDRATE);
    counterCSE7766 = new Supla::Sensor::CSE_7766(uart);
